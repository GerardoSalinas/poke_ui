...
export default function ReportsTable({ reports, loading, onRefresh, onDownload, onDelete }) {
  ...
  // Manejar el borrado del CSV
  const handleDelete = async (report) => {
    const reportId = getPropertyValue(report, "reportId")
    console.log(`borrando reporte con reportId:${reportId}`)
    const data = await onDelete(reportId)
    toast.success(`Reporte ${reportId} eliminado!`)
    onRefresh()
  }
